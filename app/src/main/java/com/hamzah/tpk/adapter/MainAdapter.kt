package com.hamzah.tpk.adapter
...
import com.hamzah.tpk.activities.DetailLocationActivity
import com.hamzah.tpk.adapter.MainAdapter.SearchViewHolder
import com.hamzah.tpk.data.model.ModelResults
import com.hamzah.tpk.utils.HitungJarak.getDistance
...
class MainAdapter(private val context: Context?) : RecyclerView.Adapter<SearchViewHolder>() {
    ...
    override fun onBindViewHolder(holder: SearchViewHolder, position: Int) {
        ...
        val modelLocation = modelResults[position].modelGeometry?.modelLocation
        val strLat = if (modelLocation != null) modelLocation.lat else null
        val strLong = modelResults[position].modelGeometry?.modelLocation!!.lng
        val strJarak = strLat?.let {
            if (strLong != null) {
                getDistance(it, strLong, strLatitude, strLongitude)
            }
        }
...
private fun <E> ArrayList<E>.addAll(elements: ArrayList<E?>) {
