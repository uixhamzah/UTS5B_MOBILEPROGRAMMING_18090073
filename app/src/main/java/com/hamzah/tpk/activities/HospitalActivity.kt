...
class HospitalActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progressDialog.show()
        listResultViewModel.getHospitalLocation().observe1(hospitalActivity = this) { modelResults: ArrayList<ModelResults?> ->
            if (modelResults.size != 0) {
                ...
                Toast.makeText(
                    this,
                    "Oops, lokasi Rumah Sakit tidak ditemukan!",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
...
private fun <T> LiveData<T>.observe(hospitalActivity: HospitalActivity, function: (ArrayList<ModelResults?>) -> Unit) {
