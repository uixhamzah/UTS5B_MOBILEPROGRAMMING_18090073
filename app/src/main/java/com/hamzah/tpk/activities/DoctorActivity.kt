package com.hamzah.tpk.activities
...
import com.hamzah.tpk.adapter.MainAdapter
import com.hamzah.tpk.data.model.ModelResults
import com.hamzah.tpk.viewmodel.ListResultViewModel
import im.delight.android.location.SimpleLocation
import kotlinx.android.synthetic.main.activity_location.*
import java.util.*
import com.hamzah.tpk.activities.observe as observe1
...
class DoctorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progressDialog.show()
        this.listResultViewModel.getDoctorLocation().observe1(this) { modelResults: ArrayList<ModelResults?> ->
            if (modelResults.size != 0) {
                mainAdapter.setResultAdapter(modelResults)
                with(progressDialog) { dismiss() }
            } else {
                Toast.makeText(
                    this,
                    "Oops, lokasi Praktik Dokter tidak ditemukan!",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
...
private fun <T> LiveData<T>.observe(doctorActivity: DoctorActivity, function: (ArrayList<ModelResults?>) -> Unit) {
